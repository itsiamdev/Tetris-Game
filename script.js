(function () {
  ...
  var tetris = {
    ...
    calcScore: function (data) {
      ...
      var shape = data.shape || false;
      var speed = data.speed || 0;
      if (lines > 0) {
        this.incScore(lines * this["level" + this.level][1]);
        this.incLines(lines);
      }
      if (shape === true) {
        this.incScore(shape * this["level" + this.level][2]);
      }
      /*if (speed > 0){ this.incScore(speed * this["level" +this .level[3]]);}*/
    },
    checkScore: function () {
    ...
    move: function (dir) {
      ...
      if (this.checkMove(tempX, tempY, this.curShape)) {
        this.curSqs.forEach(function (sq, i) {
    ...
    rotate: function () {
      if (this.curShapeIndex !== 6) {
        //square
        var temp = [];
        this.curShape.forEach(function (coord) {
          temp.push([coord[1] * -1, coord[0]]);
        });
    ...
    removeBlock: function (x, y) {
      var me = this;
      this.markBoardAt(x, y, 0);
      this.sqs = this.sqs.filter(function (sq, i) {
